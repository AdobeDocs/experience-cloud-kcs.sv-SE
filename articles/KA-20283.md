...
I det här läget misslyckas en utrullning av den överordnad sidan på grund av en konflikt i sidans miniatyrstruktur, mellan:
...
5. rulla ut sidan
6. nu går du till <b>[!DNL Sites] UI</b>
7. öppna sidegenskaperna för `/content/we-retail/us/en/women` igen
8. i *[!UICONTROL Thumbnail]* flik, klicka på *[!UICONTROL Generate Preview]* och spara ändringen
9. nu visas sidan igen i `/siteadmin`
...
Problemet inträffar under utrullning eftersom AEM inte kan skapa `dam:thumbnails` nod nedanför `jcr:content` för live copy-sida (`/content/we-retail/us/en/women/jcr:content/image/file/jcr:content`) och orsaken till det är under utrullning mixinType `dam:Thumbnails` används inte för sidnod för live-kopia `/content/we-retail/us/en/women/jcr:content/image/file/jcr:content` och `dam:Thumbnails` mappen kan bara skapas nedan `dam:Thumbnails` nodeType.

MSM-standardbeteendet är att ignorera ny blandning som lagts till i noderna vid källan under utrullning 0 och därför inte läggs till i `/content/we-retail/us/en/women/jcr:content/image/file/jcr:content`.
