...
description: Lär dig hantera filer som är skrivskyddade och oföränderliga i AEM.
...
I det här dokumentet beskrivs vilka filer som är låsta och inte ska ändras samt hur du gör de önskade konfigurationsinställningarna korrekt i Adobe Experience Manager.
...
Det aktuella värdet för `DISP_LOG_LEVEL` variabeln är info. Vi kan justera detta för att spåra eller felsöka, eller det talvärde/den nivå du väljer. Nu kan du justera loggnivån automatiskt överallt som styr loggen.
...
Vi kopierade den befintliga `aem_publish.vhost` eftersom den redan har det vi behöver för att få saker och ting att fungera och vi inte vill förnya en redan stark start.  Nu redigerar vi den nya filen weretail.vhost och gör de ändringar som behövs.
