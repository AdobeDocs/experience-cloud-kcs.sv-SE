...
Dokumentet beskriver vilka filer som är låsta i som inte kan ändras och hur du gör de önskade konfigurationsinställningarna på rätt sätt.<br><br> <br><br><br><br><br>Varför?<br><br><br><br> <br><br>
När AMS etablerar ett system kommer det att implementera en baslinjekonfiguration som gör allting funktionellt och säkert.  Det här är inte saker som AMS vill försäkra sig om som en grund för funktionalitet och säkerhet.  För att uppnå detta markeras vissa filer som skrivskyddade och oföränderliga, så att du slipper ändra dem.
...
![Visar ett bowlingband med en boll som rullar längs körfältet.  Kulan har en pil med ordet &quot;du&quot;.  Spaltmellanrummen är upphöjda och de har orden&quot;oföränderliga filer&quot; ovanför dem.](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/bowling-file-immutability.png "bowling-file-immutability")<br><br><br><br> <br><br>
...
- Övertäckning för anpassade ändringar motsvarar övertäckningsmetoder i AEM
- Du kan åtgärda AMS-konfigurationer med hjälp av ändringsanpassningar
- Testa grundinstallationen jämfört med anpassade konfigurationer kan göras samtidigt för att hjälpa till att avgöra om problemen beror på anpassningar eller något annat

<br><br><br><br>Vilka filer?<br><br><br><br> <br><br>
...
<br><br>Så här gör du ändringar<br><br><br><br> <br><br>Variabler<br><br><br><br> <br><br>
...
| `Include /etc/httpd/conf.d/variables/ams_default.vars````IfModule` `disp_apache2.c``    ``DispatcherConfig conf.dispatcher.d/dispatcher.any``    ``DispatcherLog logs/dispatcher.log``    ``DispatcherLogLevel ${DISP_LOG_LEVEL}``    ``DispatcherDeclineRoot 0``    ``DispatcherUseProcsedURL 1``/``IfModule&quot; |
| --- |

<br><br><br><br><br> <br><br>
Se hur <b>DispatcherLogLevel</b> -direktivet har en variabel av DISP_LOG_LEVEL i stället för det normalvärde som du skulle se där.  Ovanför det kodavsnittet visas även en include-sats till en variabelfil.  Variabelfilen */etc/httpd/conf.d/variables/ams_default.vars*är där vi vill titta vidare.  Här är innehållet i variabelfilen:
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br> <br><br>
Om du ser det aktuella värdet för variabeln DISP_LOG_LEVEL är info.  Vi kan justera detta för att spåra, felsöka eller för att ange ett talvärde/nivå.  Nu justeras loggnivån automatiskt överallt där detta styr.
<br><br><br><br> <br><br>Överläggsmetod<br><br><br><br> <br><br>
Vänligen förstå [inkluderingsfiler på den översta nivån](https://helpx.adobe.com/experience-manager/kb/ams-dispatcher-manual/explanation-config-files.html#include-order) eftersom det här kommer att vara din startpunkt för anpassningar.  För att börja med ett enkelt exempel har vi ett scenario där vi vill lägga till ett nytt domännamn som vi tänker peka på den här dispatchern.  Domänexemplet vi ska använda är <b>we-retail.adobe.com. </b> Vi börjar med att kopiera en befintlig konfigurationsfil till en ny där vi kan lägga till våra ändringar:
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br> <br><br>
Vi kopierade den befintliga *aem_publish.vhost* eftersom den redan har det vi behöver för att få saker och ting att fungera och vi inte vill förnya en redan stark start.  Nu redigerar vi *weretail.vhost* och göra de ändringar som behövs.
...
<br><br><br><br><br> <br><br>
Nu vet webbservern att en domän är något som ska generera trafik för, men vi måste ändå informera utgivarmodulen om att den har ett nytt domännamn som ska respekteras.  Vi börjar med att skapa en ny \*_vhost.any-fil <b>/etc/httpd/conf.dispatcher.d/vhosts/weretail_vhosts.any</b> och i den filen ska vi ange det domännamn som vi vill använda:
<br><br><br><br><br> <br><br><br><br>
