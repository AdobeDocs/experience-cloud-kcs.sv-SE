...
<b>Steg 1: Hitta administratörer och anonyma användare</b>

1. Gå till CRXDE lite app /crx/de/index.jsp och logga in som admin-användare (i källsystemet).
2. Gå till &quot;Verktyg&quot; = &quot;Fråga&quot;.
3. Ange den här frågan i rutan &quot;Fråga&quot; längst ned för att hitta administratörsanvändaren: */jcr:root/home/users//element(\*,rep:User)@rep:mainName=&quot;admin&quot; .*
4. Klicka på Kör och kopiera sökvägen till administratörsanvändarnoden i resultatet till en textfil.
5. Upprepa steg 3 med en fråga för anonym användare: */jcr:root/home/users//element(\*,rep:User)@rep:mainName=&quot;anonymous&quot; .*
6. Klicka på Kör och kopiera sökvägen till den anonyma användarnoden i resultatet till en textfil (så nu har du två sökvägar, en för&quot;admin&quot; och en för&quot;anonym&quot;).
...
<b>Steg 2A: Migrera användare och grupper (med crx2oak eller ek-upgrade)</b>
...
6. Ändra sökvägarna på raden så att de matchar din instans (lägg till *segmentgammal:* innan sökvägen, om det behövs, finns här [https://jackrabbit.apache.org/oak/docs/migration.html](https://jackrabbit.apache.org/oak/docs/migration.html)): */opt/aem-source/crx-quickstart/database/opt/aem-destination/crx-quickstart/database*
7. Kör sedan kommandot i serverns skal.     `java -Xms2g -Xmx2g -jar oak-upgrade-1.8.12.jar \`
...
<b>Steg 2B: Migrera användare och grupper (med paket)</b>
...
1. Kopiera sökvägen för anonyma noder och administratörsanvändarnoder från resultaten i steg 1 (&quot;Hitta admin och anonyma användare&quot;) ovan.
   ...
   /home/users/Q/QY5FIMXeQIbGpwZtQ3Dv - adminanvändare på datorn som jag skapar paketet på

   /home/users/K/Kj1406Qo9IDODc_nk5Ib - anonym användare på den dator där jag skapar paketet
2. Gå till &quot;Package Manager&quot;, *http://host:port/crx/packmgr/index.jsp* och logga in som administratör.
3. Skapa paketet &quot;users&quot;
4. Lägg till ett filter i paketkonfigurationen för /home/users med följande exkluderingsregler (i filtret /home/users):
...
9. Lägg till mode=&quot;merge&quot; i filtret.. -tagg, till exempel:
...
16. Om du migrerar från en äldre AEM till 6.3 går du till användargränssnittet /useradmin och lägger till användarreplikeringsmottagaren i gruppen&quot;administratörer&quot;.




<b>Steg 3. Migrera åtkomstkontrollistor</b>
...
3. Gå till http://aem-host:port/crx/packmgr/index.jsp and inloggning som administratör.
4. Klicka på ACL-paketet.
5. Klicka på Redigera.
6. Välj fliken Avancerat (se skärmbild nedan).
7. I listrutan AC-hantering väljer du Sammanfoga för att undvika att ta bort befintliga åtkomstkontrollistor i målsystemet. Detta är särskilt viktigt när du migrerar ACL:er mellan olika versioner av AEM (eftersom det inte går att ta bort ACL:er från lådan).


Om du *not* kan installera verktyg (ACS Commons) för att AEM och sedan följa dessa steg. Observera att datorn där du kör kommandona måste vara Mac OS, Linux eller Windows (med Cygwin) med cURL, python och Java SDK installerat.
...
4. Välj fliken Avancerat och ställ in AC-hanteringsläget på Sammanfoga.
5. Spara.
6. Bygg paketet och ladda ned det.
7. Kör det här kommandot på paketet för att extrahera META-INF/vault/filter.xml: 
...
17. Upprepa steg 8-16 för alla andra banor som ändrar kommandot för banvändning. Detta skulle till exempel hämta åtkomstkontrollistorna under /etc i stället för /content: 
