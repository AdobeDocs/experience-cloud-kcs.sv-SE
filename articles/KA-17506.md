...
Det finns en tidsgräns för svar, vanligtvis 10, 30 eller 60 sekunder. Svaret kan innehålla en positiv bekräftelse (`command _status = 0`) eller ett fel (se 5.1.3 *`command_status`*, *tabell 5-2* i SMPP-specifikationen för listan över standardfel). Vanligtvis är dessa svar omedelbara och ingen tidsgräns för svar nås.
...
<br><br><br><br><br><br>Initiering av SMPP-anslutning<br><br><br><br><br><br>
SMPP-anslutningen börjar med att ansluta med TCP. Sedan en `BIND` åtgärden skickas av [!DNL Campaign], bekräftas av en `BIND RESP`. Dessa åtgärder beskrivs i avsnitt 4.1 i SMPP-specifikationen (`BIND` åtgärd).
...
Inloggningen finns i `system_id` fält.



I [!DNL Campaign]bör du se en *`BIND_TRANSMITTER`* paket när en MT-överföring initieras och *`BIND_RECEIVER`* paket när *`nlsm s`* utlöser en MO/SR-anslutning.

<b>Sändare, mottagare och sändare: </b>SMPP-kontakten för Campaign Classic fungerar i ett separat sändar-/mottagarläge: det finns två TCP-anslutningar, en för överföring av MT och en annan för att ta emot MO och SR. Observera att TCP-anslutningen alltid initieras av [!DNL Campaign], även för mottagarläge.
...
<br><br><br><br><br><br>Tar emot MO<br><br><br><br><br><br>
När mottagaren är bunden kan SMS-C skicka MO när som helst. Flerlägesobjektet skickas med en *`DELIVER_SM`* PDU med bitarna 2-5 av *`esm_clas s`* klart (ofta, *`esm_class`* blir bara 0).
...
The *`DELIVER_SM`* PDU:n måste snabbt besvaras av en *`DELIVER_SM_RESP`* PDU med samma *`sequence_number`*.
<br><br><br><br><br><br>Skicka MT<br><br><br><br><br><br>
...
![smpp5](https://helpx.adobe.com/content/dam/help/en/campaign/kb/smpp-protocol-wireshark/jcr_content/main-pars/image_1891077095/smpp5.png "smpp5")<br><br><br><br><br><br>Kodning av MT<br><br><br><br><br><br>
Varning:
...
<br><br><br><br><br><br>Flera SR för samma MT<br><br><br><br><br><br>
