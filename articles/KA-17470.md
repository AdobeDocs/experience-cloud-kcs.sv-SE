...
På Linux-plattformen kan kommandot lsof användas för att felsöka vilka resurser som är öppna av processen.

Här följer ett exempelskript för att samla in användbara SLF-utdata:
...
<br><br><br><br><br> <br><br>
Inspect resultatet av den skapade filen lsof-sorted-counts-\*.txt.  Den visar vilka filer eller socketar som för närvarande är öppna av processen.
...
Om du vill ändra det maximala antalet öppna filer för användare som inte är rotfiler ändrar du filen /etc/security/limits.conf. Du kan ange gränsen per användare:
...
2. Lägg till den här raden i /etc/sysctl.conf för att öka systemets högsta tillåtna värde för öppna filer: fs.file-max = 300000
